...
function addChatItem(color, data, text, summarize) {
	...
	if(text > 4){
		$('figure.like>img').attr('src',data.profilePictureUrl);
		let nama = data.nickname;
		let arnama = nama.split(" ");
		if(arnama.length > 1 && nama.length > 14){
			nama = arnama[0] + " <span></span>";
		}else if(arnama.length > 1 && nama.length < 15){
			nama = arnama[0] + " <span>"+ arnama[1] +"</span>";
		}else{
			nama = arnama[0] + " <span></span>";
		}
		$('figure.like>figcaption>h3').html(nama);
	}
	let pattern = /shared/;
	if(pattern.test(text) == true){
		$('figure.share>img').attr('src',data.profilePictureUrl);
		let nama = data.nickname;
		let arnama = nama.split(" ");
		if(arnama.length > 1 && nama.length > 14){
			nama = arnama[0] + " <span></span>";
		...
		$('figure.share>figcaption>h3').html(nama);
	}
    /*container.append(`
...
connection.on('like', (msg) => {
    ...
    if (typeof msg.likeCount === 'number') {
		...
		addChatItem('#447dd4', msg, msg.likeCount)
    }
})
